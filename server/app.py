...
from typing import Any, Deque, Dict, List, Optional, Tuple, Set, Union
import hashlib
...
current_obs = None
fps = 80
audio_last_frame_time = None
audio_measured_fps = None
...
class AudioConnectionManager:
    """Fan out low-latency PCM chunks from the game thread to web clients."""

    def __init__(self):
        self.lock = threading.Lock()
        self.queues: Dict[int, Deque[Tuple[bytes, float]]] = {}

    async def connect(self, websocket: WebSocket):
        await websocket.accept()
        with self.lock:
            self.queues[id(websocket)] = deque(maxlen=16)
    ...
    def disconnect(self, websocket: WebSocket):
        with self.lock:
            self.queues.pop(id(websocket), None)

    def publish(self, pcm: bytes, playback_rate: float):
        if not pcm:
            return
        with self.lock:
            for audio_queue in self.queues.values():
                audio_queue.append((pcm, playback_rate))

    async def send_next(self, websocket: WebSocket) -> bool:
        with self.lock:
            audio_queue = self.queues.get(id(websocket))
            chunk = audio_queue.popleft() if audio_queue else None
        if chunk is None:
            return False

        pcm, playback_rate = chunk
        await websocket.send_text(json.dumps({
            "type": "audio-rate",
...
def step_environment(actions_pressed):
    ...
    global current_obs, audio_last_frame_time, audio_measured_fps
    ...
    with memory_lock:
        ...
        if hasattr(env, "consume_audio"):
            audio_chunk = env.consume_audio()
    ...
    # Both cores generate roughly 60 emulated frames of audio per game second.
    # Use measured throughput rather than target FPS: frame capture and state
    # work make the real loop rate fluctuate, and using the target causes
    # repeated browser underruns (audible as gaps/clicks).
    audio_now = time.perf_counter()
    if audio_last_frame_time is not None:
        audio_interval = audio_now - audio_last_frame_time
        if 0.001 < audio_interval < 1.0:
            instantaneous_fps = 1.0 / audio_interval
            if audio_measured_fps is None:
                audio_measured_fps = instantaneous_fps
            else:
                audio_measured_fps = (audio_measured_fps * 0.95) + (instantaneous_fps * 0.05)
    audio_last_frame_time = audio_now

    current_game_fps = env.get_current_fps(fps) if env else fps
    effective_fps = audio_measured_fps or current_game_fps
    audio_manager.publish(audio_chunk, max(0.25, min(6.0, effective_fps / 60.0)))
...
async def websocket_audio(websocket: WebSocket):
    """Stream stereo signed-16 PCM generated by the active emulator."""
    await audio_manager.connect(websocket)
    try:
        while True:
            try:
                sent = await audio_manager.send_next(websocket)
                if not sent:
                    await asyncio.sleep(0.004)
            except WebSocketDisconnect:
                break
            except Exception as e:
                logger.warning(f"Error in audio streaming: {e}")
                break
    finally:
