...
- **`backup_manager.py`** — Create and restore backups of `.pokeagent_cache/{run_id}/` (e.g. on objective completion). Restored zips copy persistent cache files such as the emulator checkpoint, objectives, learned stores, trajectories, and metrics.
- **`resume.py`** — Stable run-ID validation plus PokeAgent short-term checkpoint persistence/restoration. Modern checkpoints restore `agent_state.json` exactly; older runs reconstruct their last rolling history window and step counter from `trajectory_history.jsonl` and `checkpoint_llm.txt`.
